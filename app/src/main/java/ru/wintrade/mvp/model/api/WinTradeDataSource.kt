...
interface WinTradeDataSource {
    ...
    @GET("auth/users/me/")
    fun getProfile(@Header("Authorization") token: String): Single<ResponseProfile>
    ...
    fun subscribeToTrader(@Header("Authorization") token: String, @Body subscription: RequestSubscription): Single<RequestSubscription>

    @GET("api/v1/subscription/list/")
    fun mySubscriptions(@Header("Authorization") token: String): Single<List<ResponseSubscription>>
